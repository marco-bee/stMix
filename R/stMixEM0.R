...
#' @param x0 numerical vector (7x1): initial values of the parameters \emph{p},
#' \eqn{\mu_1,} \eqn{\sigma_1}, \eqn{\nu_1}, 
#' \eqn{\mu_2}, \eqn{\sigma_2}, \eqn{\nu_2}. 
...
#' @export
#' @examples
#' y <- rfstMix(100,.5,1,1.3,0.5,3,1,1,1,7)
#' x <- c(.6,1,0.8,3,1.2,.9,9)
#' res <- stMixEM0(y,.6,1,0.8,3,1.2,.9,9,1000)
#'
#' @importFrom Rdpack reprompt

stMixEM0 <- function(Y,p1,mu1,sigma1,nu1,mu2,sigma2,nu2,maxiter)
{
  p2 <- 1 - p1
  n <- length(Y)
  nit <- 1
  ...
  while (2>1)
  {
    ...
    res2 <- optim(c(mu2,log(sigma2),pmin(log(nu2),1.7e+307)),
          llstWei0, gr = NULL, control = list(fnscale = -1), Y, post2)
    mu2 <- res2$par[1]
    ...
    if (nit > maxiter || change < epsilon)
      break
    nit <- nit + 1
    results <- list(p=p1,post=post1,mu1=mu1,sigma1=sigma1,nu1=nu1,
     mu2=mu2,sigma2=sigma2,nu2=nu2,nit=nit,loglik=loglik)
  }
  return(results)
}

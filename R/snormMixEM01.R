#' Skewed-normal mixture estimation via EM
#'
...
#' \eqn{\mu_1,} \eqn{\sigma_1}
#' \eqn{\gamma_2}, \eqn{\mu_2}, \eqn{\sigma_2}. 
...
#' @export
#' @examples
#' y <- rfstMix(100,.5,1,1.3,0.5,.8,1,1)
#' x0 <- c(.6,1.3,1,0.8,1.2,.7,.9)
#' res <- stMixEM(y,x0)
#'
#' @importFrom Rdpack reprompt

snormMixEM01 <- function(Y,p1,mu1,sigma1,gamma2,mu2,sigma2,maxiter)
{
  p2 <- 1 - p1
  n <- length(Y)
  nit <- 1
  ...
  while (2>1)
  {
    ...
    res1 <- optim(c(mu1,log(sigma1)),
            llsnormWei0, gr = NULL, control = list(fnscale = -1), Y, post1)
    mu1 <- res1$par[1]
    ...
    loglik <- sum(log(dfsnormMix(Y,p1,1,mu1,sigma1,gamma2,mu2,sigma2)))
    ...
    if (nit > maxiter || change < epsilon)
      break
    nit <- nit + 1
    results <- list(p=p1,post=post1,mu1=mu1,sigma1=sigma1,
     gamma2=gamma2,mu2=mu2,sigma2=sigma2,nit=nit,loglik=loglik)
  }
  return(results)
}

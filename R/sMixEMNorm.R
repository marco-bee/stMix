...
#' skewed normal distributions by means of the EM algorithm.
#' @param x0 numerical vector (7x1): initial values of the parameters \emph{p},
#' \eqn{\gamma_1}, \eqn{\mu_1,} \eqn{\sigma_1},
...
#' res <- stMixEMNorm(y,x0,1000)
#'
#' @importFrom Rdpack reprompt

sMixEMNorm <- function(Y,p1,gamma1,mu1,sigma1,gamma2,mu2,sigma2,maxiter)
{
  p2 <- 1 - p1
  n <- length(Y)
  nit <- 1
  ...
  while (2>1)
  {
    ...
    loglik <- sum(log(dfsnormMix(Y,p1,gamma1,mu1,sigma1,gamma2,mu2,sigma2)))
    ...
    if (nit > maxiter || change < epsilon)
      break
    nit <- nit + 1
    results <- list(p=p1,post=post1,gamma1=gamma1,mu1=mu1,sigma1=sigma1,

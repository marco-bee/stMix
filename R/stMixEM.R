...
#' @details If the initial value of \eqn{\gamma_1} and/or \eqn{\gamma_2} is
#' NULL, the corresponding parameter is not estimated, but set equal to 1.
#' @export
#' @examples
#' y <- rfstMix(100,.5,1.2,1.3,0.5,3,.8,1,1,7)
#' x <- c(.6,1.3,1,0.8,3,1.2,.7,.9,9)
#' res <- stMixEM(y,.6,1.3,1,0.8,3,1.2,.7,.9,9,100,1000)
#'
#' @importFrom Rdpack reprompt

stMixEM <- function(Y,p1,gamma1,mu1,sigma1,nu1,gamma2,mu2,
                    sigma2,nu2,cap=30,maxiter)
{
  p2 <- 1 - p1
  n <- length(Y)
  nit <- 1
  ...
  while (2>1)
  {
    ...
    if (is.null(gamma1) & !is.null(gamma2))
    {
      ...
      f1 <- dfst(Y,gamma1_used,mu1,sigma1,nu1)
      f2 <- dfst(Y,gamma2,mu2,sigma2,nu2)
      f <- p1 * f1 + p2 * f2
      loglik <- sum(log(f))
      ...
      sigma2 <- exp(res2$par[3])
      nu2 <- pmin(exp(res2$par[4]),cap)
      loglik <- sum(log(dfstMix(Y,p1,gamma1_used,mu1,sigma1,nu1,gamma2,mu2,sigma2,nu2)))
    ...
    {
      gamma2_used <- 1
      parold <- c(p1,gamma1,mu1,sigma1,mu2,sigma2)
      
      # evaluate component densities and mixture density
      
      f1 <- dfst(Y,gamma1,mu1,sigma1,nu1)
      f2 <- dfst(Y,gamma2_used,mu2,sigma2,nu2)
      f <- p1 * f1 + p2 * f2
      loglik <- sum(log(f))
      ...
      sigma1 <- exp(res1$par[3])
      nu1 <- pmin(exp(res1$par[4]),cap)
      res2 <- optim(c(mu2,log(sigma2),pmin(log(nu2),cap)),
                    llstWei, gr = NULL, control = list(fnscale = -1), Y, post2)
      mu2 <- res2$par[1]
      sigma2 <- exp(res2$par[2])
      nu2 <- pmin(exp(res2$par[3]),cap)
      loglik <- sum(log(dfstMix(Y,p1,gamma1,mu1,sigma1,nu1,gamma2_used,mu2,sigma2,nu2)))
      
      # check convergence
      
      diffpar <- parold - c(p1,gamma1,mu1,sigma1,mu2,sigma2)
      change <- max(abs(diffpar))
      if (nit > maxiter || change < epsilon)
        break
      nit <- nit + 1
      estPars <- c(p1,gamma1,mu1,sigma1,nu1,mu2,sigma2,nu2)
      results <- list(estPars=estPars,post=post1,nit=nit,loglik=loglik)
    }
    if (is.null(gamma1) & is.null(gamma2))
    {
      gamma1_used <- 1
      gamma2_used <- 1
      parold <- c(p1,mu1,sigma1,mu2,sigma2)
      ...
      sigma2 <- exp(res2$par[2])
      nu2 <- pmin(exp(res2$par[3]),cap)
      loglik <- sum(log(dfstMix(Y,p1,gamma1_used,mu1,sigma1,nu1,gamma2_used,mu2,sigma2,nu2)))
      
      # check convergence
      
      diffpar <- parold - c(p1,mu1,sigma1,mu2,sigma2)
      change <- max(abs(diffpar))
      if (nit > maxiter || change < epsilon)
        break
      nit <- nit + 1

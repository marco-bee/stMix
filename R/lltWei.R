...
#'
#' This function evaluates the log-likelihood of a location-scale t 
#' distribution with weighted observations.
...
lltWei <- function(x,y,post)
{
  mu <- x[1]
  sigma <- exp(x[2])
  nu <- exp(x[3])
  ll <- dlst(y,nu,mu,sigma,log = TRUE)
  llik <- sum(ll*post)
  return(llik)
}

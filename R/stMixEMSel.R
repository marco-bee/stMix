...
stMixEMSel <- function(Y,p1,gamma1,mu1,sigma1,nu1,gamma2,mu2,sigma2,nu2,maxiter)
{
  p2 <- 1 - p1
  n <- length(Y)
  nit <- 1
  ...
  # start iterations
    ...
    while (2>1)
    {
      ...
      if (nit > maxiter || change < epsilon)
        break
      nit <- nit + 1
      results11 <- list(p=p1,post=post1,gamma1=gamma1,mu1=mu1,sigma1=sigma1,nu1=nu1,
    ...
    while (2>1)
    {
      ...
      res1 <- optim(c(mu1,log(sigma1),pmin(log(nu1),1.7e+307)),
                    lltWei, gr = NULL, control = list(fnscale = -1), Y, post1)
      mu1 <- res1$par[1]
      ...
      if (nit > maxiter || change < epsilon)
        break
      nit <- nit + 1
      results01 <- list(p=p1,post=post1,gamma1=gamma1,mu1=mu1,sigma1=sigma1,nu1=nu1,
                      gamma2=gamma2,mu2=mu2,sigma2=sigma2,nu2=nu2,nit=nit,loglik=loglik)
  }
  while (2>1)
  {
      ...
      sigma1 <- exp(res1$par[3])
      nu1 <- exp(res1$par[4])
      res2 <- optim(c(mu2,log(sigma2),pmin(log(nu2),1.7e+307)),
                    lltWei, gr = NULL, control = list(fnscale = -1), Y, post2)
      mu2 <- res2$par[1]
      sigma2 <- exp(res2$par[2])
      nu2 <- exp(res2$par[3])
      loglik <- sum(log(dfstMix(Y,p1,gamma1,mu1,sigma1,nu1,1,mu2,sigma2,nu2)))
      ...
      if (nit > maxiter || change < epsilon)
        break
      nit <- nit + 1
      results10 <- list(p=p1,post=post1,gamma1=gamma1,mu1=mu1,sigma1=sigma1,nu1=nu1,
                gamma2=gamma2,mu2=mu2,sigma2=sigma2,nu2=nu2,nit=nit,loglik=loglik)
  }
    while (2>1)
    {
      ...
      sigma2 <- exp(res2$par[2])
      nu2 <- exp(res2$par[3])
      loglik <- sum(log(dfstMix(Y,p1,1,mu1,sigma1,nu1,1,mu2,sigma2,nu2)))
      ...
      if (nit > maxiter || change < epsilon)
        break
      nit <- nit + 1
      results00 <- list(p=p1,post=post1,gamma1=gamma1,mu1=mu1,sigma1=sigma1,nu1=nu1,
                        gamma2=gamma2,mu2=mu2,sigma2=sigma2,nu2=nu2,nit=nit,loglik=loglik)
  }
  retMat <- list(results11,results01,results10,results00)
}
